...
class PPDF extends FPDF{
		...
		function Header(){
			...
			$this->ln(5);
			$this->CuadroCabecera(32,'Fecha de Reporte'.": ",50,utf8_encode($fecha));
			$this->Pagina();
			$this->ln(5);
		...
		function Footer()
		{	global $lema,$idioma,$DatosGenerador,$title;
			...
			if($this->CurOrientation=="P"){
				$this->Cell((round(($this->ancho-50)/2)+0),3,utf8_decode($title.""),$BordePie,0,"R");
				$this->ln();
